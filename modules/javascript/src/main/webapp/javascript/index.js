...
atmosphere = {
    ...
    AtmosphereRequest: function (options) {
         ...
         */
        function _close() {
            atmosphere.util.debug("Closing (AtmosphereRequest._close() called)");
        ...
        function _clearState() {
            ...
            if (_websocket != null) {
                if (_websocket.canSendMessage) {
                    atmosphere.util.debug("invoking .close() on WebSocket object");
                    _websocket.close();
                }
                _websocket = null;
            }
        ...
        function _subscribe(options) {
            ...
            logger = createLogger(_request.logLevel);
                ...
                open: function () {
                    ...
                    function poll() {
                        ...
                        script.onload = script.onreadystatechange = function () {
                            atmosphere.util.debug("jsonp.onload");
                        ...
                        script.onerror = function () {
                            atmosphere.util.debug("jsonp.onerror");
                            request.scriptCount = 1;
                            script.clean();
                        };
                    ...
                    window[callback] = function (msg) {
                        atmosphere.util.debug("jsonp.window");
        ...
        function _executeSSE(sseOpened) {
            ...
            _sse.onopen = function (event) {
                atmosphere.util.debug("sse.onopen");
                _timeout(_request);
            ...
            _sse.onmessage = function (message) {
                atmosphere.util.debug("sse.onmessage");
                _timeout(_request);
            ...
            _sse.onerror = function (message) {
                atmosphere.util.debug("sse.onerror");
                clearTimeout(_request.id);
        ...
        function _executeWebSocket(webSocketOpened) {
            ...
            _websocket.onopen = function (message) {
                atmosphere.util.debug("websocket.onopen");
                _timeout(_request);
                offline = false;
            ...
            _websocket.onmessage = function (message) {
                atmosphere.util.debug("websocket.onmessage");
                _timeout(_request);
            ...
            _websocket.onerror = function (message) {
                atmosphere.util.debug("websocket.onerror");
                clearTimeout(_request.id);
            ...
            _websocket.onclose = function (message) {
                atmosphere.util.debug("websocket.onclose");
                clearTimeout(_request.id);
        ...
        function _handleProtocol(request, message) {
            ...
            if (request.enableProtocol && request.firstMessage && trim(message).length !== 0) {
                ...
                request.uuid = trim(messages[pos]);
                ...
                _heartbeatInterval = parseInt(trim(messages[pos + 1]), 10);
            ...
            if (rq.force || (rq.reconnect && (rq.maxRequest === -1 || rq.requestCount++ < rq.maxRequest))) {
                ...
                if (rq.transport !== 'polling') {
                    ...
                    ajaxRequest.onabort = function () {
                        atmosphere.util.debug("ajaxrequest.onabort")
                        _invokeClose(true);
                    };

                    ajaxRequest.onerror = function () {
                        atmosphere.util.debug("ajaxrequest.onerror")
                ...
                ajaxRequest.onreadystatechange = function () {
                    atmosphere.util.debug("ajaxRequest.onreadystatechange, new state: " + ajaxRequest.readyState);
                    if (_abortingConnection) {
                        atmosphere.util.debug("onreadystatechange has been ignored due to _abortingConnection flag");
                        return;
                    }
                    ...
                    if (update) {
                        ...
                        if (rq.transport === 'long-polling' && trim(responseText).length === 0) {
        ...
        function _configureXDR(request) {
            ...
            var handle = function (xdr) {
                ...
                if (transport !== 'polling') {
                    ...
                    if (transport === 'long-polling' && trim(message).length === 0)
                        return;
            ...
            switch (response.state) {
                ...
                case "closed":
                    ...
                    if (!closed) {
                        ...
                        atmosphere.util.debug("Request already closed, not firing onClose (" + response.state + " case)");
                    }
...
atmosphere.util = {
    ...
    warn(...args) {
        atmosphere.util.log(WARN, args);
    },

    info(...args) {
        atmosphere.util.log(INFO, args);
    },
    ...
    error() {
        atmosphere.util.log(ERROR, ...args);
    },
    ...
    checkCORSSupport: function () {
        ...
        // KreaTV 4.1 -> 4.4
        else if (trim(navigator.userAgent).slice(0, 16) === "KreaTVWebKit/531") {
            return true;
        }
        // KreaTV 3.8
        else if (trim(navigator.userAgent).slice(-7).toLowerCase() === "kreatel") {
            return true;
        }
